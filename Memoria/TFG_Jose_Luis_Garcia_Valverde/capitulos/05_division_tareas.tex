...
\begin{itemize}
    ...
    \item \textbf{ShiftRequest}: Permutas solicitadas. Status progresa: PENDING → APPROVED/REJECTED.
\end{itemize}
...
\begin{enumerate}
    \item Acceder a propiedad inexistente: \texttt{user.sedeId.name} → TypeError si sedeId es null
    \item Pasar tipo incorrecto a API: enviar email string cuando API espera number → falla silenciosa en backend
...
\begin{itemize}
    ...
    \item \textbf{Props Drilling:} Pasar state a través de componentes intermedios. Simple pero \texttt{<App> → <Navbar> → <UserMenu> → <LogoutButton>} es tedioso.
...
\begin{enumerate}
    ...
    \item Si refresh es v\u00e1lido, obtiene nuevo access token → usuario redirigido a dashboard (sesión persistió).
...
\begin{itemize}
    \item ✓ Admin no conoce contraseña real del empleado (segregación de deberes)
    \item ✓ Empleado no puede elegir contraseña débil (temp es robusta por defecto)
    \item ✓ Previene ataques de diccionario (contraseña random de 10 chars tiene $10^{10}$ combinaciones vs human-chosen típicamente 3 palabras)
    \item  Requiere que admin comunique temp password (fuera de band). Riesgo si interceptado.
\end{itemize}

\textbf{Alternativa Rechazada}: Admin elige contraseña inicial.
\begin{itemize}
    \item ✓ Menos pasos
    \item ✗ Admin podría espiar contraseña del empleado (desconfianza)
    \item ✗ Contraseña podría ser débil, predecible
\end{itemize}
...
\textbf{Auditoría}: Cada cambio de contraseña registra timestamp, IP, user agent. Crítico para investigar si contraseña fue cambiada sin consentimiento.
...
\begin{enumerate}
    \item \textbf{Nivel de Componente (ProtectedRoute)}: Si usuario no está autenticado, redirige a login. Si está autenticado pero su rol no está en lista de roles requeridos, muestra ``Acceso denegado''.
...
\begin{itemize}
    ...
    \item Frontend es UX layer. Protege UI para que interfaz no se rompa o no muestre opciones invalidas.
\end{itemize}

Nunca confiar en frontend para seguridad. Atacante puede abrir DevTools, falsificar localStorage, enviar requests sin pasar por UI.\newline
...
Alternativa: mostrar error. Pero UX es peor (usuario confundido), y login no es una ``error'', es un estado normal.
